...
if __name__ == '__main__':
    for i in [5, 10, 20, 25]: #    for i in [1, 2, 5, 10, 20, 25]: for i in [2, 5, 10, 20, 25]:
        for calc_type in ['', 'mean_', 'mean_std_']:
            ...
            train_merged = pd.read_csv(
                "../Sprungdaten_processed/without_preprocessed/percentage/" + str(i) + "/vector_percentage_" + calc_type + str(
                    i) + "_train.csv")
            ...
            for estimator in [100]:
                for depth in [1, 2]:    #for depth in [1, 2]:
